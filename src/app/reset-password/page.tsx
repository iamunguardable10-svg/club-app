    ...
    <main className="os-page">
      <div className="os-container max-w-md space-y-5">
        <header className="os-hero p-6">
          ...
          <h1 className="os-title mt-2">Choose a new password</h1>
        </header>
        {account === undefined ? <p className="text-sm text-slate-400">One moment …</p> : null}
        {account === null ? (
          <section className="os-panel grid gap-3 p-5 text-sm text-slate-300">
            <p>This link is no longer valid. Request a new one from the sign-in page.</p>
            <a href="/login" className="os-success justify-center">Go to sign in</a>
          </section>
        ) : null}
        {account ? (
          <form onSubmit={submit} className="os-panel grid gap-4 p-5">
            <p className="text-sm text-slate-400">For {account.email}</p>
            <label className="grid gap-1.5 text-sm font-bold text-slate-200">
              ...
              <span className="text-xs font-medium text-slate-500">At least 8 characters.</span>
            </label>
