        ...
        {createWarning ? (
          <div className="mt-4 rounded-xl border border-amber-500/30 bg-amber-950/20 p-3">
            ...
            {createMatch?.candidate.scope === 'department_only' ? (
              <button type="button" onClick={() => handleMakeFacilityGlobal(createMatch.candidate.id, newFacilityDepartmentIds)} disabled={isSaving} className="mt-3 rounded-lg bg-amber-300 px-3 py-2 text-xs font-black text-slate-950 hover:bg-amber-200 disabled:opacity-50">Make existing hall global</button>
            ) : null}
          </div>
        ) : null}
      ...
      <form onSubmit={handleAssignFacility} className="rounded-3xl border border-slate-800 bg-slate-950/70 p-5">
        ...
        <div className="mt-4 space-y-4">
          <select value={selectedFacilityId} onChange={(event) => setSelectedFacilityId(event.target.value)} className="w-full rounded-xl border border-slate-700 bg-slate-900 px-4 py-3 text-sm outline-none focus:border-violet-400">
            {globalFacilities.map((facility) => <option key={facility.id} value={facility.id}>{facility.name} — {facility.address || 'No address'}</option>)}
          </select>
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,#064E3B_0,#070A12_45%)] px-4 py-8 text-white sm:px-8">
      <div className="mx-auto max-w-6xl space-y-5">
        <section className="rounded-3xl border border-slate-800 bg-slate-950/80 p-6 shadow-sm">
          <Link href={backTarget.href} className="inline-flex items-center text-sm font-black text-emerald-300 hover:text-emerald-200">{backTarget.label}</Link>
          <div className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <p className="mt-3 max-w-3xl text-sm leading-6 text-slate-400">Manage shared club facilities and department-only locations separately.</p>
            </div>
            <button type="button" onClick={() => setIsEditMode((current) => !current)} className={isEditMode ? 'w-fit rounded-xl bg-emerald-400 px-4 py-3 text-sm font-black text-slate-950 transition hover:bg-emerald-300' : 'w-fit rounded-xl border border-emerald-500/70 px-4 py-3 text-sm font-black text-emerald-200 transition hover:bg-emerald-950/40'}>{isEditMode ? 'Done editing' : 'Edit facilities'}</button>
          </div>
        </section>
        ...
        <section className="rounded-3xl border border-slate-800 bg-slate-950/70 p-5">
          <div className="flex items-end justify-between gap-3">
            <div><p className="text-xs font-black uppercase tracking-[0.18em] text-emerald-300">Global facilities</p><h2 className="mt-2 text-xl font-black">Shared club facilities</h2></div>
          ...
          <div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {globalFacilities.map((facility) => (
              <article key={facility.id} className="rounded-xl border border-slate-800 bg-slate-900/60 px-4 py-3 transition hover:border-emerald-400 hover:bg-emerald-950/20">
                <Link href={`/admin/facilities/${facility.id}/calendar?from=facilities`} className="block"><p className="font-black text-white">{facility.name}</p><p className="mt-1 text-xs text-slate-500">{facility.address || 'No address set yet'}</p></Link>
                {isEditMode ? <button type="button" onClick={() => handleDeleteFacility(facility.id)} disabled={isSaving} className="mt-3 rounded-lg border border-red-500/60 px-2.5 py-1.5 text-xs font-black text-red-200 hover:bg-red-950/40 disabled:opacity-50">Delete facility</button> : null}
              </article>
            ))}
              ...
              return (
                <section key={department.id} className="rounded-2xl border border-slate-800 bg-slate-900/60 p-4">
                  <button type="button" onClick={() => toggleExpandedDepartment(department.id)} className="flex w-full items-center justify-between gap-3 text-left">
                    <span><span className="block font-black">{department.name}</span><span className="mt-1 block text-xs font-bold text-slate-500">{sharedAssignments.length} shared assigned · {departmentOnlyFacilities.length} department-only</span></span>
                          ...
                          {sharedAssignments.length > 0 ? sharedAssignments.map((assignment) => {
                            ...
                            return (
                              <div key={assignment.id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-800 bg-slate-950/60 px-3 py-2">
                                <Link href={`/admin/facilities/${assignment.facility_id}/calendar?from=facilities`} className="text-sm font-bold text-slate-200 hover:text-emerald-300">{facility?.name ?? 'Unknown facility'}{facility?.address ? <span className="block text-xs text-slate-500">{facility.address}</span> : null}</Link>
                                {isEditMode ? <button type="button" onClick={() => handleRemoveAssignment(assignment.id)} disabled={isSaving} className="text-xs font-bold text-red-300 hover:text-red-200 disabled:opacity-50">Remove</button> : null}
                              </div>
                            );
