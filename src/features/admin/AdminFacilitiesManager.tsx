...
export function AdminFacilitiesManager() {
  ...
  if (state === 'loading') {
    return <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4 text-white"><section className="rounded-3xl border border-slate-800 bg-slate-950/80 p-6 text-center"><p className="text-sm font-bold text-slate-300">Loading facilities...</p></section></main>;
  }

  if (state === 'no_admin_membership') {
    return <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4 text-white"><section className="max-w-xl rounded-3xl border border-slate-800 bg-slate-950/80 p-6"><p className="text-xs font-black uppercase tracking-[0.22em] text-slate-400">Facilities</p><h1 className="mt-3 text-3xl font-black">No admin club found</h1><p className="mt-3 text-sm leading-6 text-slate-400">Create a club first before managing facilities.</p><Link href="/onboarding/create-club" className="mt-5 inline-block rounded-xl bg-emerald-400 px-4 py-3 text-sm font-black text-slate-950">Create club setup</Link></section></main>;
  }

  if (state === 'error') {
    return <main className="flex min-h-screen items-center justify-center bg-slate-950 px-4 text-white"><section className="max-w-xl rounded-3xl border border-red-900/70 bg-red-950/30 p-6"><p className="text-xs font-black uppercase tracking-[0.22em] text-red-300">Facility error</p><h1 className="mt-3 text-3xl font-black">Could not load facilities</h1><p className="mt-3 text-sm leading-6 text-red-100">{error}</p></section></main>;
  }

  const editTools = isEditMode ? (
    <section className="grid gap-5 lg:grid-cols-[0.9fr_1.1fr]">
      <form onSubmit={handleCreateFacility} className="os-section">
        <p className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Create</p><h2 className="mt-2 text-xl font-black">Add facility</h2>
        {createWarning ? <div className="mt-4 rounded-xl border border-slate-800 bg-slate-900/50 p-3"><p className="text-sm font-bold leading-6 text-slate-200">{createWarning}</p>{createMatch?.candidate.scope === 'department_only' ? <button type="button" onClick={() => handleMakeFacilityGlobal(createMatch.candidate.id, newFacilityDepartmentIds)} disabled={isSaving} className="mt-3 rounded-lg bg-amber-300 px-3 py-2 text-xs font-black text-slate-950 hover:bg-amber-200 disabled:opacity-50">Make existing hall global</button> : null}</div> : null}
        <div className="mt-4 space-y-4">
          ...
          <div><p className="text-sm font-bold text-slate-200">Departments</p><div className="mt-2 space-y-2">{departments.map((department) => <label key={department.id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-800/90 bg-slate-950/50 px-4 py-3 text-sm ring-1 ring-white/[0.03]"><span className="font-bold text-slate-100">{department.name}</span><input type="checkbox" checked={newFacilityDepartmentIds.includes(department.id)} onChange={() => toggleNewFacilityDepartment(department.id)} className="h-4 w-4" /></label>)}</div></div>
          <button type="submit" disabled={isSaving} className="os-success w-full justify-center disabled:cursor-not-allowed disabled:opacity-60">Add facility</button>
        </div>
      </form>
      <form onSubmit={handleAssignFacility} className="os-section">
        <p className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Assign</p><h2 className="mt-2 text-xl font-black">Assign shared facility</h2>
        <div className="mt-4 space-y-4"><select value={selectedFacilityId} onChange={(event) => setSelectedFacilityId(event.target.value)} className="os-field focus:border-sky-300 focus:ring-sky-400/10">{globalFacilities.map((facility) => <option key={facility.id} value={facility.id}>{facility.name} — {facility.address || 'No address'}</option>)}</select><div className="space-y-2">{departments.map((department) => { const alreadyAssigned = assignedDepartmentIdsForSelectedFacility.has(department.id); const checked = selectedDepartmentIds.includes(department.id) || alreadyAssigned; return <label key={department.id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-800/90 bg-slate-950/50 px-4 py-3 text-sm ring-1 ring-white/[0.03]"><span><span className="font-bold text-slate-100">{department.name}</span>{alreadyAssigned ? <span className="ml-2 text-xs font-bold text-slate-400">already assigned</span> : null}</span><input type="checkbox" checked={checked} disabled={alreadyAssigned} onChange={() => toggleDepartment(department.id)} className="h-4 w-4" /></label>; })}</div><button type="submit" disabled={isSaving || departments.length === 0 || globalFacilities.length === 0 || selectedDepartmentIds.length === 0} className="os-primary w-full justify-center disabled:cursor-not-allowed disabled:opacity-60">Assign selected</button></div>
      </form>
    </section>
  ) : null;
  ...
  return (
    <main className="os-page px-4 py-8 text-white sm:px-8">
      <div className="os-container max-w-6xl space-y-5">
        <section className="os-hero"><Link href={backTarget.href} className="inline-flex items-center text-sm font-black text-slate-400 hover:text-emerald-200">{backTarget.label}</Link><div className="mt-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-black uppercase tracking-[0.24em] text-slate-400">Facilities</p><h1 className="mt-3 text-3xl font-black tracking-tight sm:text-5xl">Facilities for {club?.name}</h1></div><button type="button" onClick={() => setIsEditMode((current) => !current)} className={isEditMode ? 'os-success w-fit px-4 py-3' : 'os-secondary w-fit px-4 py-3'}>{isEditMode ? 'Done editing' : 'Edit facilities'}</button></div></section>
        ...
        <section className="os-section"><div className="flex items-end justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Club facilities</p><h2 className="mt-2 text-xl font-black">Shared facilities</h2></div><span className="text-sm font-bold text-slate-400">{globalFacilities.length} global</span></div><div className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-3">{globalFacilities.map((facility) => <article key={facility.id} className="rounded-2xl border border-slate-800/90 bg-slate-950/45 px-4 py-3 ring-1 ring-white/[0.03] transition hover:-translate-y-0.5 hover:border-sky-400/45 hover:bg-slate-950/75"><Link href={`/admin/facilities/${facility.id}/calendar?from=facilities`} className="block"><p className="font-black text-white">{facility.name}</p><p className="mt-1 text-xs text-slate-500">{facility.address || 'No address set yet'}</p></Link>{isEditMode ? <button type="button" onClick={() => handleDeleteFacility(facility.id)} disabled={isSaving} className="mt-3 rounded-lg border border-red-500/60 px-2.5 py-1.5 text-xs font-black text-red-200 hover:bg-red-950/40 disabled:opacity-50">Delete facility</button> : null}</article>)}{globalFacilities.length === 0 ? <p className="text-sm text-slate-500">No shared facilities yet.</p> : null}</div></section>

        <section className="os-section"><p className="text-xs font-black uppercase tracking-[0.18em] text-sky-300">Department facilities</p><h2 className="mt-2 text-xl font-black">Department access</h2><div className="mt-5 space-y-3">{departments.map((department) => {
          ...
          return <section key={department.id} className="rounded-2xl border border-slate-800/90 bg-slate-950/45 p-4 ring-1 ring-white/[0.03]"><button type="button" onClick={() => toggleExpandedDepartment(department.id)} className="flex w-full items-center justify-between gap-3 text-left"><span><span className="block font-black">{department.name}</span><span className="mt-1 block text-xs font-bold text-slate-500">{sharedAssignments.length} shared assigned · {departmentOnlyFacilities.length} department-only</span></span><span className="text-sm font-black text-sky-300">{isExpanded ? 'Hide' : 'Show'}</span></button>{isExpanded ? <div className="mt-4 grid gap-3 lg:grid-cols-2"><div className="rounded-2xl border border-slate-800/90 bg-slate-950/55 p-3 ring-1 ring-white/[0.03]"><p className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">Shared access</p><div className="mt-3 space-y-2">{sharedAssignments.length > 0 ? sharedAssignments.map((assignment) => { const facility = facilityById.get(assignment.facility_id); return <div key={assignment.id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-800/90 bg-slate-950/50 px-3 py-2 ring-1 ring-white/[0.03]"><Link href={`/admin/facilities/${assignment.facility_id}/calendar?from=facilities&departmentId=${department.id}`} className="text-sm font-bold text-slate-200 hover:text-slate-400">{facility?.name ?? 'Unknown facility'}{facility?.address ? <span className="block text-xs text-slate-500">{facility.address}</span> : null}</Link>{isEditMode ? <button type="button" onClick={() => handleRemoveAssignment(assignment.id)} disabled={isSaving} className="text-xs font-bold text-red-300 hover:text-red-200 disabled:opacity-50">Remove</button> : null}</div>; }) : <p className="text-sm text-slate-500">No shared access.</p>}</div>{isEditMode ? <div className="mt-4 rounded-xl border border-emerald-500/20 bg-emerald-950/10 p-3"><p className="text-xs font-black uppercase tracking-[0.14em] text-slate-400">Assign shared halls</p>{availableShared.length > 0 ? <div className="mt-3 grid gap-2">{availableShared.map((facility) => { const checked = selectedShared.includes(facility.id); return <label key={facility.id} className="flex cursor-pointer items-center justify-between gap-3 rounded-xl border border-slate-800/90 bg-slate-950/50 px-3 py-2 text-sm ring-1 ring-white/[0.03] transition hover:border-sky-400/45"><span className="min-w-0"><span className="block truncate font-black text-slate-100">{facility.name}</span><span className="mt-0.5 block truncate text-xs font-bold text-slate-500">{facility.address || 'No address'}</span></span><input type="checkbox" checked={checked} onChange={() => toggleDepartmentSharedSelection(department.id, facility.id)} className="h-4 w-4" /></label>; })}</div> : <p className="mt-3 text-sm text-slate-500">No further halls.</p>}<button type="button" onClick={() => assignDepartmentSharedSelections(department.id)} disabled={isSaving || selectedShared.length === 0} className="mt-3 w-full rounded-lg bg-sky-300 px-3 py-2 text-xs font-black text-slate-950 transition hover:bg-sky-200 disabled:cursor-not-allowed disabled:opacity-50">Assign selected shared halls</button></div> : null}</div><div className="rounded-2xl border border-slate-800/90 bg-slate-950/55 p-3 ring-1 ring-white/[0.03]"><p className="text-xs font-black uppercase tracking-[0.16em] text-slate-400">Department-only</p><div className="mt-3 space-y-2">{departmentOnlyFacilities.length > 0 ? departmentOnlyFacilities.map((facility) => <div key={facility.id} className="rounded-xl border border-slate-800/90 bg-slate-950/50 px-3 py-2 ring-1 ring-white/[0.03]"><Link href={`/admin/facilities/${facility.id}/calendar?from=facilities&departmentId=${department.id}`} className="block hover:text-slate-100"><p className="text-sm font-black text-slate-100">{facility.name}</p><p className="mt-1 text-xs text-slate-500">{facility.address || 'No address'}</p></Link>{isEditMode ? <div className="mt-2 flex flex-wrap gap-2"><button type="button" onClick={() => handleMakeFacilityGlobal(facility.id)} disabled={isSaving} className="rounded-lg border border-violet-500/60 px-2.5 py-1.5 text-xs font-black text-violet-200 hover:bg-violet-950/40 disabled:opacity-50">Make global</button><button type="button" onClick={() => handleDeleteFacility(facility.id)} disabled={isSaving} className="rounded-lg border border-red-500/60 px-2.5 py-1.5 text-xs font-black text-red-200 hover:bg-red-950/40 disabled:opacity-50">Delete</button></div> : null}</div>) : <p className="text-sm text-slate-500">No local halls.</p>}</div>{isEditMode ? <div className="mt-4 rounded-xl border border-violet-500/20 bg-violet-950/10 p-3"><p className="text-xs font-black uppercase tracking-[0.14em] text-slate-400">Create local hall</p><div className="mt-3 grid gap-2"><input value={draft.name} onChange={(event) => updateDepartmentOnlyDraft(department.id, { name: event.target.value })} placeholder="Hall name" className="rounded-xl border border-slate-700/90 bg-slate-950/70 px-3 py-2 text-sm text-slate-100 outline-none transition placeholder:text-slate-600 focus:border-sky-300 focus:ring-2 focus:ring-sky-400/10" /><input value={draft.address} onChange={(event) => updateDepartmentOnlyDraft(department.id, { address: event.target.value })} placeholder="Address" className="rounded-xl border border-slate-700/90 bg-slate-950/70 px-3 py-2 text-sm text-slate-100 outline-none transition placeholder:text-slate-600 focus:border-sky-300 focus:ring-2 focus:ring-sky-400/10" /><button type="button" onClick={() => createDepartmentOnlyFacility(department.id)} disabled={isSaving || !draft.name.trim() || !draft.address.trim()} className="rounded-lg bg-sky-300 px-3 py-2 text-xs font-black text-slate-950 transition hover:bg-sky-200 disabled:cursor-not-allowed disabled:opacity-50">Create local hall</button></div></div> : null}</div></div> : null}</section>;
        })}</div></section>
      </div>
    </main>
  );
