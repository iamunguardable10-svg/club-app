...
export function FacilityCalendar({ facilityId, from, departmentId, teamId }: FacilityCalendarProps) {
  ...
  const firstDepartmentTeamId = departmentId ? (teams.find((team) => team.department_id === departmentId)?.id ?? null) : null;
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm font-bold text-slate-200">
                  {formatTimeRange(draft.startsAt, draft.endsAt)} | {teamById.get(draft.teamId ?? '')?.name ?? 'Team not set'} | {facility?.name}
                </p>
