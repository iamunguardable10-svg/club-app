  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/80 px-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="facility-conflict-title">
      <div className="w-full max-w-lg rounded-3xl border border-slate-700 bg-slate-950 p-5 shadow-2xl">
        <p className="text-xs font-black uppercase tracking-[0.18em] text-amber-300">{t('conflict.kicker')}</p>
        <h2 id="facility-conflict-title" className="mt-3 text-2xl font-black text-white">{t('conflict.title')}</h2>
        <p className="mt-3 text-sm font-bold leading-6 text-slate-300">{description}</p>

        {suggestions.length > 0 ? (
          <div className="mt-5 space-y-2">
            <p className="text-xs font-black uppercase tracking-[0.16em] text-slate-500">{t('conflict.smartMoves')}</p>
        ...
        <div className={`mt-5 grid gap-3 ${facilityCalendarHref ? 'sm:grid-cols-4' : 'sm:grid-cols-3'}`}>
          ...
          {facilityCalendarHref ? (
            ...
            >
              {facilityCalendarLabel ?? t('conflict.facilityCalendar')}
            </a>
          ) : null}
          <button
            type="button"
