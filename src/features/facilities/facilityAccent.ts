/**
 * Stable accent colour per hall, so the same hall is recognisable across the
 * hall list, team settings and calendars without storing a colour.
 *
 * Ported unchanged from `src/shared/lib/facilities/accent.ts` in `543775f`.
 * Seeded by the hall id, so renaming a hall keeps its colour.
 */
