...
| 20 | Apple-Kalender verbinden (CalDAV) (erledigt, Run 33; **optional**, die App kommt komplett ohne aus) | Spieler/Trainer verbinden iCloud mit Apple-ID und **app-spezifischem Passwort**; das normale Passwort wird nie verlangt. **Geführte Anleitung in der App** (entschieden 2026-09-25): erst was die Verbindung macht und was nicht (liest nur gewählte Kalender, schreibt nur in „Club OS“, jederzeit trennbar, Passwort verschlüsselt), dann Schritt für Schritt mit Bildern/Links: appleid.apple.com öffnen → Anmeldung und Sicherheit → App-spezifische Passwörter → „Club OS“ als Namen → Passwort kopieren → in der App einfügen; danach Test der Verbindung mit klarer Fehlermeldung (falsches Passwort, 2-Faktor fehlt) und Hinweis, wie man es bei Apple widerruft. Das Passwort liegt **verschlüsselt** (Supabase Vault) und ist nur für die Abgleich-Funktion lesbar; „Trennen“ löscht es sofort, bei Apple jederzeit widerrufbar. **App → Apple:** die App legt in iCloud einen Kalender „Club OS“ an und hält ihn aktuell (neu, geändert, abgesagt). **Apple → App:** nach dem Verbinden zeigt die App die Liste aller Apple-Kalender; **je Kalender**: **„Nicht importieren“** (Standard; die App liest ihn nicht) oder **„Importieren“** – importierte Termine sind private Termine, was der Trainer davon sieht, steht in 21b. Entschieden 2026-09-25: **keine Art „Training“ beim Import** – eigenes Training legt man in der App an (einzeln oder als Serie, Stück 22) und trägt dort die Belastung nach. Andere Kalender werden nur gelesen, nie verändert. Abgleich alle 15 Minuten (pg_cron → Edge Function) und beim Öffnen der App | groß |
| 21a | Trainer sieht eigenes Training (erledigt, Run 28/29) | Rollen mit `viewAthletePlans` sehen das geplante eigene Training der Spieler (entschieden 2026-09-25: **eigenes Training immer sichtbar**), aber **nur in der Detailansicht eines Spielers** („Own training · next 2 weeks“). **Kein Hinweis beim Planen und keine Nachricht an den Trainer**, wenn eine Einheit mit eigenem Training kollidiert: das eigene Training zu verschieben oder abzusagen (mit Grund) ist Sache des Spielers (entschieden 2026-09-25). Rollen ohne das Recht sehen weiterhin nichts davon | mittel |
| 21b | Was der Trainer von privaten Terminen sieht (später; bis dahin sieht der Trainer sie nicht, entschieden 2026-09-25) | **Je importiertem Kalender** wählbar: **gar nicht / nur „belegt“ / mit Titel** (Standard: nur „belegt“; entschieden 2026-09-25: wer möchte, dass der Trainer sieht, was man macht, wählt „mit Titel“). Nicht importierte Kalender sieht niemand. Sichtbar wie 21a nur in der Detailansicht des Spielers, kein Hinweis beim Planen. Braucht 20 | klein |
