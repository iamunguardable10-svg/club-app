-- Facility identity should not depend on name alone.
--
-- Product logic:
-- - Names like Main Hall, Gym or Court 1 are weak identifiers.
-- - Address/street is the stronger signal for duplicate warnings.
-- - Therefore, facilities in one club may share a name when they are truly different locations.

alter table public.facilities
  drop constraint if exists facilities_club_id_name_key;

create index if not exists idx_facilities_club_id_name on public.facilities(club_id, name);

-- The initial setup RPC previously used ON CONFLICT (club_id, name), which depended
-- on the old unique constraint. Recreate the function without that conflict target.
...
begin
  ...
  insert into public.clubs (name, slug, city, country, created_by)
  values (
    trim(p_club_name),
    v_club_slug,
  ...
  values (v_user_id, v_club_id, null, 'club_admin', 'active');

  foreach v_department_name in array p_department_names loop
    if nullif(trim(v_department_name), '') is not null then
      insert into public.departments (club_id, name, sport)
      values (v_club_id, trim(v_department_name), trim(v_department_name))
  ...
  foreach v_facility_name in array p_facility_names loop
    if nullif(trim(v_facility_name), '') is not null then
      insert into public.facilities (club_id, name, scope, owner_department_id)
      values (v_club_id, trim(v_facility_name), 'club_shared', null)
  ...
  if p_team_department_name is not null and array_length(p_team_names, 1) is not null then
    ...
    foreach v_team_name in array p_team_names loop
      if nullif(trim(v_team_name), '') is not null then
        insert into public.teams (club_id, department_id, name, sport, season)
        values (v_club_id, v_selected_department_id, trim(v_team_name), p_team_department_name, null)
  ...
  insert into public.activity_events (club_id, actor_id, event_type, title, body)
  values (v_club_id, v_user_id, 'club_created', 'Club created', 'Initial club setup was completed.');
